...
// From address: use EMAIL_FROM_ADDRESS env var if set, otherwise default to support@myncel.com
const DEFAULT_FROM = process.env.EMAIL_FROM_ADDRESS || 'Myncel <support@myncel.com>';

const EMAIL_ADDRESSES = {
  admin: DEFAULT_FROM,
  support: DEFAULT_FROM,
  info: DEFAULT_FROM,
  contact: DEFAULT_FROM,
};
...
export async function sendTeamInviteEmail(
  ...
  const roleLabel = role === 'TECHNICIAN' ? 'Technician' : role === 'OPERATOR' ? 'Operator' : role === 'EMPLOYEE' ? 'Employee' : role === 'ADMIN' ? 'Admin' : role === 'MEMBER' ? 'Member' : role;
  return sendEmail({
    to: email,
