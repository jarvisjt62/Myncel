...
- [x] NEXTAUTH_SECRET generation instructions documented; generate a fresh value with `openssl rand -base64 32` and store it only in Vercel environment variables.
...
- [ ] Add `NEXTAUTH_SECRET`: generate a fresh value with `openssl rand -base64 32` and keep it only in Vercel environment variables
