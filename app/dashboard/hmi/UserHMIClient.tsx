...
function MachineImage({ category, machineName, status, className = '', liveData, compact = false, onCommand }: {
...
// ── Toast Notification ─────────────────────────────────────────────────────────
function Toast({ msg, type, onDone }: { msg: string; type: 'success' | 'error' | 'info' | 'warning'; onDone: () => void }) {
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <div className="p-2.5">
        ...
        {/* Machine SCADA Image */}
        <div className="w-full h-40 mb-1.5 rounded-lg overflow-hidden" style={{ background: 'transparent' }}>
        ...
        <div className="mt-2 pt-1.5 flex justify-between items-center" style={{ borderTop: '1px solid var(--card-border-b)' }}>
          <span className="text-[8px]" style={{ color: 'var(--text-ultralow)' }}>{machine.totalHours.toLocaleString()}h</span>
          <div className="flex items-center gap-1">
            {machine.gatewayOnline && (
...
function MachinePanel({
  ...
  // Track whether real gateway data is available
  const [isGatewayLive, setIsGatewayLive] = useState(false);
  const [lastGatewayReadingAt, setLastGatewayReadingAt] = useState<string | null>(null);

  // Poll real sensor readings from gateway every 6 seconds when machine is OPERATIONAL
  useEffect(() => {
    if (!live) {
    ...
    const fetchSensorReadings = async () => {
      try {
        const res = await fetch(`/api/machines/${machine.id}/sensor-readings`);
        if (!res.ok || cancelled) return;
        const data = await res.json();

        if (!data.isLive || !data.liveData) {
          // No recent gateway data — fall through to simulation
          setIsGatewayLive(false);
          return;
        }

        // Real gateway data is available — use it
        setIsGatewayLive(true);
        setLastGatewayReadingAt(data.lastReadingAt);

        const { temp: t, load: l, rpm: r, pressure: p } = data.liveData;

        if (t != null) {
          setTemp(t);
          setTempHist(h => [...h.slice(-19), t]);
        }
        if (l != null) {
          setLoad(l);
          setLoadHist(h => [...h.slice(-19), l]);
        }
        if (r != null) setRpm(r);
        if (p != null) setPressure(p);
      } catch {
    ...
    fetchSensorReadings();
    const gatewayPoll = setInterval(fetchSensorReadings, 6000);

    return () => {
      cancelled = true;
      clearInterval(gatewayPoll);
    };
  }, [live, machine.id]);

  // Simulation fallback — only runs when no live gateway data is present
  useEffect(() => {
    if (!live || isGatewayLive) {
      return;
    }
    const iv = setInterval(() => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const msg = command === 'REQUEST_MAINTENANCE'
          ...
          : `Command sent.`;
        showToast(msg, command === 'EMERGENCY_STOP' ? 'warning' : 'success');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm p-3 modal-safe-pad" style={{ backgroundColor: 'rgba(0,0,0,0.7)' }} onClick={onClose}>
      ...
      >
        ...
        <div className={`mx-4 mt-0 rounded-xl border overflow-hidden ${currentCfg.border}`} style={{ background: 'transparent' }}>
          <MachineImage category={machine.category} machineName={machine.name} status={currentStatus} className="w-full h-96" liveData={live ? { temp, load, rpm, pressure } : undefined} onCommand={sendCommand} />
        </div>
        ...
        <div className="p-4 grid md:grid-cols-3 gap-4">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              {live && (
                <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1 ${isGatewayLive ? 'bg-emerald-500/15 text-emerald-400' : 'bg-purple-500/15 text-purple-400'}`}>
                  <span className={`w-1.5 h-1.5 rounded-full inline-block ${isGatewayLive ? 'bg-emerald-400 animate-pulse' : 'bg-purple-400'}`}/>
                  {isGatewayLive ? 'GATEWAY LIVE' : 'SIMULATED'}
                </span>
              )}
            </div>
            ...
            {live && !isGatewayLive && (
              <div className="rounded-lg px-3 py-2 text-[10px]" style={{ backgroundColor: 'var(--bg-surface)', color: 'var(--text-muted)', borderColor: 'var(--border)', border: '1px solid' }}>
                <p className="font-semibold mb-0.5" style={{ color: 'var(--text-secondary)' }}>📡 No gateway connected</p>
                <p>Showing simulated data. <a href="/dashboard/gateway-setup" className="text-[#635bff] hover:underline">Connect an edge gateway</a> to see real sensor readings.</p>
              </div>
            )}
            {live && isGatewayLive && lastGatewayReadingAt && (
              <div className="rounded-lg px-3 py-2 text-[10px]" style={{ backgroundColor: 'rgba(52,211,153,0.05)', borderColor: 'rgba(52,211,153,0.2)', border: '1px solid' }}>
                <p className="text-emerald-400 font-semibold">✅ Live gateway data</p>
                <p style={{ color: 'var(--text-muted)' }}>Last reading: {new Date(lastGatewayReadingAt).toLocaleTimeString()}</p>
              </div>
            )}
          </div>
