...
export const metadata = {
  title: 'MTConnect CNC Gateway Setup — Myncel Docs',
  description: 'Pull CNC machine status, execution state, spindle load, alarms, and production data from MTConnect agents.',
};
    ...
    <div className="min-h-screen bg-[var(--bg-page)]">
      <header className="sticky top-0 z-20 border-b border-[var(--border)] bg-[var(--bg-surface)] px-6 py-4">
        <div className="mx-auto flex max-w-5xl flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-bold text-[var(--text-primary)]">MTConnect CNC</span>
          </div>
      ...
      <main className="mx-auto max-w-5xl space-y-6 px-6 py-8">
        <section className="rounded-3xl border border-[var(--border)] bg-[var(--bg-surface)] p-8 shadow-sm">
          ...
          <h1 className="mt-4 text-4xl font-bold text-[var(--text-primary)]">MTConnect CNC</h1>
          <p className="mt-4 max-w-3xl text-base leading-7 text-[var(--text-secondary)]">Pull CNC machine status, execution state, spindle load, alarms, and production data from MTConnect agents.</p>
        </section>
        ...
        <section className="grid gap-6 lg:grid-cols-2">
          <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-surface)] p-6 shadow-sm">
            ...
            <p className="mt-3 text-sm leading-6 text-[var(--text-secondary)]">Use MTConnect for CNC machines and controllers that expose /current or /sample XML endpoints.</p>
            ...
            <ul className="mt-3 list-disc space-y-2 pl-5 text-sm leading-6 text-[var(--text-secondary)]">
              <li>Confirm the MTConnect agent URL from the CNC vendor or shop network.</li>
              <li>Map data_item names to the machine metrics you want in Myncel.</li>
              <li>Use /sample for time-series windows and /current for latest state.</li>
            </ul>
          </div>
