...
export const metadata = {
  title: 'Rockwell EtherNet/IP Gateway Setup — Myncel Docs',
  description: 'Read named tags from Allen-Bradley / Rockwell PLCs through EtherNet/IP gateway patterns.',
};
    ...
    <div className="min-h-screen bg-[var(--bg-page)]">
      <header className="sticky top-0 z-20 border-b border-[var(--border)] bg-[var(--bg-surface)] px-6 py-4">
        <div className="mx-auto flex max-w-5xl flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-bold text-[var(--text-primary)]">Rockwell EtherNet/IP</span>
          </div>
      ...
      <main className="mx-auto max-w-5xl space-y-6 px-6 py-8">
        <section className="rounded-3xl border border-[var(--border)] bg-[var(--bg-surface)] p-8 shadow-sm">
          ...
          <h1 className="mt-4 text-4xl font-bold text-[var(--text-primary)]">Rockwell EtherNet/IP</h1>
          <p className="mt-4 max-w-3xl text-base leading-7 text-[var(--text-secondary)]">Read named tags from Allen-Bradley / Rockwell PLCs through EtherNet/IP gateway patterns.</p>
        </section>
        ...
        <section className="grid gap-6 lg:grid-cols-2">
          <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-surface)] p-6 shadow-sm">
            ...
            <p className="mt-3 text-sm leading-6 text-[var(--text-secondary)]">Use Rockwell EtherNet/IP when telemetry is available as controller tags such as MotorAmps, CycleCount, or FaultCode.</p>
            ...
            <ul className="mt-3 list-disc space-y-2 pl-5 text-sm leading-6 text-[var(--text-secondary)]">
              <li>Use read-only tags or a dedicated telemetry program where possible.</li>
              <li>Validate tag names and types in Studio 5000.</li>
              <li>Keep scan rates slower than control-critical PLC logic.</li>
            </ul>
          </div>
