...
// Uses the qrcode package so generated labels are scannable by phones and barcode scanners.
async function generateQRDataUrl(text: string, size: number = 120): Promise<string> {
...
interface LabelProps {
  ...
  qrDataUrl: string;
  onEnlarge?: (machine: Machine, qrDataUrl: string) => void;
}

function MachineLabel({ machine, appUrl, size, showSerial, showLocation, showStatus, showManufacturer, qrDataUrl, onEnlarge }: LabelProps) {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start', flex: 1 }}>
        ...
        <div
          onClick={() => onEnlarge?.(machine, qrDataUrl)}
          style={{
            flexShrink: 0, padding: '3px', backgroundColor: '#fff', border: '1px solid #e2e8f0', borderRadius: '6px',
            cursor: onEnlarge ? 'pointer' : 'default',
            transition: 'transform 0.15s ease, box-shadow 0.15s ease',
          }}
          onMouseEnter={e => { if (onEnlarge) { (e.currentTarget as HTMLElement).style.transform = 'scale(1.08)'; (e.currentTarget as HTMLElement).style.boxShadow = '0 4px 12px rgba(0,0,0,0.15)'; } }}
...
export default function QRLabelsClient({ machines }: { machines: Machine[] }) {
  ...
  const qrPixelSize = labelSize.id === 'lg' ? 90 : labelSize.id === 'md' ? 75 : 60;
    ...
    const buildQrCodes = async () => {
      const entries = await Promise.all(
        machines.map(async (machine) => {
          ...
          const dataUrl = await generateQRDataUrl(qrUrl, qrPixelSize * 3);
          return [machine.id, dataUrl] as const;
        })
      );
      if (!cancelled) setQrDataUrls(Object.fromEntries(entries));
    };
    buildQrCodes().catch(error => {
      console.error('Failed to generate QR labels:', error);
    });
    return () => { cancelled = true; };
  }, [machines, appUrl, qrPixelSize]);

  const handleEnlarge = (machine: Machine, qrDataUrl: string) => {
    setEnlargedQR({ machine, qrDataUrl });
  };
  ...
  const qrFallback = 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="120" height="120"><rect width="120" height="120" fill="white"/><text x="60" y="62" text-anchor="middle" font-size="10" fill="#64748b">Generating</text></svg>');
              ...
              <div className="p-6">
                ...
                <div ref={printRef} style={{ display: 'none' }}>
                  {selectedMachines.map(m => {
                    const qrDataUrl = qrDataUrls[m.id] || qrFallback;
                      ...
                      <div key={m.id} style={{
                        ...
                        <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start', flex: 1 }}>
                          <div style={{ flexShrink: 0, padding: '3px', border: '1px solid #e2e8f0', borderRadius: '6px' }}>
                            <img src={qrDataUrl} alt="QR" style={{ display: 'block', width: qrPixelSize, height: qrPixelSize }} />
                          </div>
                          <div style={{ flex: 1, minWidth: 0, overflow: 'hidden' }}>
        ...
        >
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <h3 style={{ fontSize: 16, fontWeight: 700, color: '#0a2540', margin: 0 }}>
                {enlargedQR.machine.name}
              </h3>
              <button
                onClick={() => setEnlargedQR(null)}
                style={{ fontSize: 20, background: 'none', border: 'none', cursor: 'pointer', color: '#64748b', padding: 0, lineHeight: 1 }}
              >
                ×
              </button>
            </div>

            <div style={{ display: 'inline-block', padding: 12, backgroundColor: '#fff', border: '2px solid #e2e8f0', borderRadius: 12 }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={enlargedQR.qrDataUrl}
                alt="Enlarged QR"
            ...
            <div style={{ marginTop: 16 }}>
              <div style={{ fontSize: 13, color: '#0a2540', fontWeight: 600 }}>{enlargedQR.machine.name}</div>
              {enlargedQR.machine.manufacturer && (
                <div style={{ fontSize: 12, color: '#546884', marginTop: 2 }}>
                  {enlargedQR.machine.manufacturer}{enlargedQR.machine.model ? ` · ${enlargedQR.machine.model}` : ''}
                </div>
              )}
              {enlargedQR.machine.serialNumber && (
                <div style={{ fontSize: 12, color: '#546884', fontFamily: 'monospace', marginTop: 2 }}>
                  S/N: {enlargedQR.machine.serialNumber}
                </div>
              )}
              {enlargedQR.machine.location && (
                <div style={{ fontSize: 12, color: '#546884', marginTop: 2 }}>
                  📍 {enlargedQR.machine.location}
                </div>
              )}
            ...
            <p style={{ fontSize: 11, color: '#94a3b8', marginTop: 12 }}>
              Scan this QR code with any phone camera to open the equipment page
            </p>
          </div>
        </div>
      )}
