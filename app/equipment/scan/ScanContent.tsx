...
export default function BarcodeScanContent() {
  ...
  const extractMachineLookupCode = (rawCode: string) => {
    const trimmed = rawCode.trim();
    try {
      const parsed = new URL(trimmed);
      const match = parsed.pathname.match(/\/equipment\/([^/?#]+)/);
      if (match?.[1]) return decodeURIComponent(match[1]);
    } catch {
      // Not a URL; continue with raw scanner value.
    }
    const directPathMatch = trimmed.match(/\/equipment\/([^/?#]+)/);
    if (directPathMatch?.[1]) return decodeURIComponent(directPathMatch[1]);
    return trimmed;
  };

  const lookupMachine = async (code: string) => {
    const lookupCode = extractMachineLookupCode(code);
    setLoading(true);
    setNotFound(false);
    setMachine(null);
    setError('');
    ...
    try {
      // Search machines by scanned equipment URL, machine ID, serial number, or name
      const res = await fetch(`/api/machines?search=${encodeURIComponent(lookupCode)}&limit=1`);
      if (res.ok) {
