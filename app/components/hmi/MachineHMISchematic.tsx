...
function ES({ onClick, active = false }: { onClick: () => void; active?: boolean }) {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => setB(x => !x), 600);
    return () => clearInterval(t);
  }, [active]);

  const handleClick = () => {
    if (active) return; // Already triggered — machine is in BREAKDOWN
    if (!window.confirm('⚠️ EMERGENCY STOP\n\nThis will set the machine to BREAKDOWN status and create a critical alert and work order.\n\nProceed?')) return;
    onClick();
  };
  ...
  return (
    <button onClick={handleClick}
      aria-pressed={active}
      disabled={active}
      title={active ? 'Emergency stop is active — machine is in BREAKDOWN' : 'Trigger emergency stop'}
      style={{ background: active && b ? '#ef4444' : active ? '#7f1d1d' : '#991b1b', border: '2px solid #ef4444', color: '#fff',
        borderRadius: 3, padding: '4px 10px', fontSize: 9, fontWeight: 900,
        cursor: active ? 'not-allowed' : 'pointer',
        letterSpacing: 1, fontFamily: 'monospace',
        boxShadow: active ? '0 0 12px #ef4444cc' : '0 0 4px #ef444440',
        opacity: active ? 0.7 : 1,
        transition: 'background 0.2s, box-shadow 0.2s' }}>E-STOP</button>
  );
}
        ...
        {!compact && (
          <div style={{ width:150, padding:10, borderLeft:`1px solid ${c.panelBorder}`, display:'flex', flexDirection:'column', gap:4 }}>
            ...
            <div style={{marginTop:6,display:'flex',flexWrap:'wrap',gap:4}}>
              ...
              <ES active={isB} onClick={()=>onCommand?.('EMERGENCY_STOP','EMERGENCY STOP')}/>
            </div>
          </div>
        )}
      </div>
