...
export default function GlobalSearch() {
  ...
  const performSearch = async (searchQuery: string, navigateFirst = false) => {
    const trimmed = searchQuery.trim();
    if (trimmed.length < 2) return null;

    setIsLoading(true);
    try {
      const response = await fetch(`/api/search?q=${encodeURIComponent(trimmed)}&types=pages,machines,work-orders,parts,maintenance`);
      if (response.ok) {
        ...
        if (navigateFirst) {
          const allResults: SearchResult[] = [
            ...(nextResults?.pages || []),
            ...(nextResults?.machines || []),
            ...(nextResults?.workOrders || []),
            ...(nextResults?.parts || []),
            ...(nextResults?.maintenanceTasks || []),
          ];
          if (allResults[0]) {
  ...
  const navigateToResult = (result: SearchResult) => {
    // When selecting another dashboard result while already on /dashboard, update the
    // URL immediately and notify DashboardClient. This avoids a same-page router timing
    // issue where the tab changes but the selected entity modal does not open.
    if (typeof window !== 'undefined' && result.url.startsWith('/dashboard') && window.location.pathname === '/dashboard') {
      window.history.pushState(null, '', result.url);
      window.dispatchEvent(new HashChangeEvent('hashchange'));
      window.dispatchEvent(new Event('myncel-search-navigation'));
      return;
    }
  ...
  const handleSelectResult = (index: number) => {
    ...
    if (allResults[index]) {
      navigateToResult(allResults[index]);
      setIsOpen(false);
      setQuery('');
      setResults(null);
    }
        ...
        <div className="absolute top-full left-0 mt-2 w-96 max-h-[70vh] bg-white rounded-xl shadow-xl border border-gray-200 z-50 overflow-hidden">
          <div className="p-2">
            {results.pages && results.pages.length > 0 && (
              <div className="mb-2">
                <div className="px-3 py-1.5 text-xs font-semibold text-gray-500 uppercase tracking-wide">Pages</div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-500 truncate">{page.subtitle}</p>
                      </div>
                    </button>
                  );
                })}
            ...
            {results.machines && results.machines.length > 0 && (
              <div className="mb-2">
                ...
                {results.machines.map((machine, idx) => {
                  const globalIndex = (results.pages?.length || 0) + idx;
                  return (
                    <button
                      key={machine.id}
                      onClick={() => handleSelectResult(globalIndex)}
            ...
            {results.workOrders && results.workOrders.length > 0 && (
              <div className="mb-2">
                ...
                {results.workOrders.map((wo, idx) => {
                  const globalIndex = (results.pages?.length || 0) + (results.machines?.length || 0) + idx;
                  return (
                    <button
                      key={wo.id}
                      onClick={() => handleSelectResult(globalIndex)}
            ...
            {results.parts && results.parts.length > 0 && (
              <div className="mb-2">
                ...
                {results.parts.map((part, idx) => {
                  const globalIndex = (results.pages?.length || 0) + (results.machines?.length || 0) + (results.workOrders?.length || 0) + idx;
                  return (
                    <button
                      key={part.id}
                      onClick={() => handleSelectResult(globalIndex)}
            ...
            {results.maintenanceTasks && results.maintenanceTasks.length > 0 && (
              <div>
                ...
                {results.maintenanceTasks.map((task, idx) => {
                  const globalIndex = (results.pages?.length || 0) + (results.machines?.length || 0) + (results.workOrders?.length || 0) + (results.parts?.length || 0) + idx;
                  return (
                    <button
                      key={task.id}
                      onClick={() => handleSelectResult(globalIndex)}
