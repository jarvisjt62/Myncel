...
export default function SensorSimulator() {
  ...
  const [autoScroll, setAutoScroll] = useState(true); // User-controllable auto-scroll
  ...
  useEffect(() => {
    // Only auto-scroll the logs container, not the outer page
    // Use scrollTop manipulation to avoid affecting parent scroll containers
    if (autoScroll && logsContainerRef.current) {
      const container = logsContainerRef.current;
      const isNearBottom = container.scrollHeight - container.scrollTop - container.clientHeight < 100;
      
      if (isNearBottom || !userScrolledUpRef.current) {
        // Use direct scrollTop assignment instead of scrollIntoView
        // This ensures only the logs container scrolls, not the outer page
        container.scrollTop = container.scrollHeight;
      }
    }
  }, [logs, autoScroll]);

  // Handle user scroll in the logs container
  // Using a ref to track scroll state without causing re-renders on every scroll event
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-0">
        ...
        <div className="lg:col-span-2 flex flex-col">
          ...
          <div 
            ...
            className="flex-1 overflow-y-auto max-h-96 p-4 space-y-1 font-mono text-xs">
            {logs.length === 0 ? (
