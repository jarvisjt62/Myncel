...
const ProductsMenu = ({ isOpen, onClose }: MegaMenuProps) => (
  <div className={`absolute top-full bg-white border border-[#e6ebf1] rounded-2xl shadow-2xl mt-2 transition-all duration-200 z-50 w-[1024px] max-w-[calc(100vw-2rem)] ${isOpen ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 -translate-y-2 pointer-events-none'}`}
...
const SolutionsMenu = ({ isOpen, onClose }: MegaMenuProps) => (
  <div className={`absolute top-full bg-white border border-[#e6ebf1] rounded-2xl shadow-2xl mt-2 transition-all duration-200 z-50 w-[920px] ${isOpen ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 -translate-y-2 pointer-events-none'}`}
    style={{ left: '50%', transform: isOpen ? 'translateX(-38%) translateY(0)' : 'translateX(-38%) translateY(-8px)' }}>
    <div className="p-6 grid grid-cols-3 gap-6">
      ...
      <div>
        <div className="text-xs font-bold text-[#8898aa] uppercase tracking-wider mb-3">New Sectors</div>
        <div className="space-y-3 mb-5">
          {[
            { name: 'Hotels & Commercial Buildings', desc: 'Generators, HVAC, elevators, water systems', icon: '🏨', href: '/solutions/hotels' },
            { name: 'Hospitals & Healthcare', desc: 'Critical equipment, compliance, uptime', icon: '🏥', href: '/solutions/hospitals' },
            { name: 'Warehouses & Cold Storage', desc: 'Refrigeration, conveyors, dock equipment', icon: '📦', href: '/solutions/warehouses' },
            { name: 'Oil & Gas Operations', desc: 'Pumps, compressors, generators, field assets', icon: '🛢️', href: '/solutions/oil-gas' },
          ].map(item => (
            <Link key={item.name} href={item.href} onClick={onClose} className="flex items-start gap-2.5 group">
              <span className="text-lg leading-none mt-0.5">{item.icon}</span>
              <div>
      ...
      <div>
        <div className="text-xs font-bold text-[#8898aa] uppercase tracking-wider mb-3">By Region</div>
        <div className="space-y-3 mb-5">
          {[
            { name: 'United States', desc: 'Manufacturing, healthcare, logistics', icon: '🇺🇸', href: '/locations/united-states' },
            { name: 'Canada', desc: 'Plants, warehouses, energy operations', icon: '🇨🇦', href: '/locations/canada' },
            { name: 'Europe', desc: 'GDPR-ready maintenance operations', icon: '🇪🇺', href: '/locations/europe' },
            { name: 'West Africa', desc: 'Generator-first reliability for harsh conditions', icon: '🌍', href: '/locations/west-africa' },
          ].map(item => (
            <Link key={item.name} href={item.href} onClick={onClose} className="flex items-start gap-2.5 group">
              <span className="text-lg leading-none mt-0.5">{item.icon}</span>
              <div>
        ...
        <div className="bg-[#f6f9fc] rounded-xl p-4 border border-[#e6ebf1]">
          ...
          <p className="text-xs text-[#425466] mb-2">Browse all solution pages or start a free trial.</p>
          <div className="flex items-center gap-3">
            <Link href="/solutions" onClick={onClose} className="text-xs font-semibold text-[#635bff] hover:underline">All solutions →</Link>
            <Link href="/free-trial" onClick={onClose} className="text-xs font-semibold text-[#635bff] hover:underline">Free trial →</Link>
          </div>
        </div>
      </div>
    </div>
      ...
      <div>
        ...
        <div className="space-y-3 mb-4">
          {[
            ...
            { name: 'Partners', desc: 'Referral, reseller & technology partners', icon: '🤝', href: '/partners' },
          ].map(item => (
            <Link key={item.name} href={item.href} onClick={onClose} className="flex items-start gap-2.5 group">
              <span className="text-lg leading-none mt-0.5">{item.icon}</span>
              <div>
        ...
        <div className="lg:hidden border-t border-[#e6ebf1] bg-white max-h-[calc(100vh-80px)] overflow-y-auto">
          <div className="px-4 py-4 space-y-2">
            ...
            <MobileMenuSection title="Solutions" items={[
              ...
              { label: 'Hotels & Commercial Buildings', href: '/solutions/hotels' },
              { label: 'Hospitals & Healthcare', href: '/solutions/hospitals' },
              { label: 'Warehouses & Cold Storage', href: '/solutions/warehouses' },
              { label: 'Oil & Gas Operations', href: '/solutions/oil-gas' },
              { label: 'Small Shops', href: '/solutions/small' },
              { label: 'Mid-size Plants', href: '/solutions/midsize' },
            ]} onClose={() => setMobileOpen(false)} />
            
            {/* Locations Section */}
            <MobileMenuSection title="Locations" items={[
              { label: 'United States', href: '/locations/united-states' },
              { label: 'Canada', href: '/locations/canada' },
              { label: 'Europe', href: '/locations/europe' },
              { label: 'West Africa', href: '/locations/west-africa' },
            ]} onClose={() => setMobileOpen(false)} />
