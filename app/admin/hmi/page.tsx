...
// ── Toast Notification ────────────────────────────────────────────────────────
function AdminToast({ msg, type, onDone }: { msg: string; type: 'success' | 'error' | 'info' | 'warning'; onDone: () => void }) {
  useEffect(() => {
  ...
  const bg = type === 'success' ? 'bg-emerald-500/20 border-emerald-500/40 text-emerald-300'
           ...
           : type === 'warning' ? 'bg-orange-500/20 border-orange-500/40 text-orange-300'
           :                      'bg-blue-500/20 border-blue-500/40 text-blue-300';
  const icon = type === 'success' ? '✅' : type === 'error' ? '❌' : type === 'warning' ? '🚨' : 'ℹ️';
  return (
  ...
  const sendCommand = async (command: string, label: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        const sideEffectErrors = Array.isArray(data.sideEffectErrors) ? data.sideEffectErrors : [];
        let msg = '';
        if (sideEffectErrors.length > 0 && data.message) {
          msg = data.message;
        } else if (command === 'REQUEST_MAINTENANCE') {
          msg = data.workOrder?.woNumber
            ? `Maintenance requested. WO ${data.workOrder.woNumber} created. Status -> MAINTENANCE`
            : `Maintenance requested. Status -> MAINTENANCE`;
        } else if (command === 'EMERGENCY_STOP') {
          msg = `Emergency Stop triggered successfully. Status -> BREAKDOWN.`;
        ...
        setToast({ msg, type: sideEffectErrors.length > 0 ? 'info' : command === 'EMERGENCY_STOP' ? 'warning' : 'success' });
      ...
      <div className="[background:var(--bg-surface-2)] border border-[var(--border)] rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="space-y-4">
            ...
            <a href={`/admin/machines?machineId=${encodeURIComponent(machine.id)}`} className="block w-full text-center px-3 py-2 text-xs font-semibold text-[#635bff] border border-[#635bff]/30 rounded-lg hover:bg-[#635bff]/10 transition-colors">
...
function HMIPageInner() {
  ...
  const [simOrgId, setSimOrgId] = useState<string>('');
  const [simMachineId, setSimMachineId] = useState<string>('');
  ...
  const orgOptions = useMemo(() => Array.from(
    machines.reduce((map, machine) => {
      const orgId = machine.organizationId || 'unknown';
      if (!map.has(orgId)) {
        map.set(orgId, {
          id: orgId,
          name: machine.organization?.name || 'Unknown',
          count: 0,
        });
      }
  ...
  ).map(([, org]) => org).sort((a, b) => a.name.localeCompare(b.name)), [machines]);

  const orgNames = orgOptions.map(org => org.name);
  const selectedFilterOrgName = filterOrg === 'ALL' ? '' : (orgOptions.find(org => org.id === filterOrg)?.name || 'Unknown');
  const simOrgMachines = machines.filter(m => m.organizationId === simOrgId);
  const selectedSimOrg = orgOptions.find(org => org.id === simOrgId) || null;
  const selectedSimMachine = machines.find(m => m.id === simMachineId) || null;

  useEffect(() => {
    if (orgOptions.length === 0) {
      if (simOrgId) setSimOrgId('');
      if (simMachineId) setSimMachineId('');
      return;
    }

    const orgStillExists = orgOptions.some(org => org.id === simOrgId);
    if (!simOrgId || !orgStillExists) {
      const firstOrgId = orgOptions[0].id;
      setSimOrgId(firstOrgId);
      const firstMachine = machines.find(m => m.organizationId === firstOrgId);
      setSimMachineId(firstMachine?.id || '');
      return;
    }

    const machineStillInOrg = machines.some(m => m.id === simMachineId && m.organizationId === simOrgId);
    if (!simMachineId || !machineStillInOrg) {
      const firstMachine = machines.find(m => m.organizationId === simOrgId);
      setSimMachineId(firstMachine?.id || '');
    }
  }, [machines, orgOptions, simOrgId, simMachineId]);

  const runSim = async (scenario: string) => {
    if (!simOrgId || !simMachineId) {
      setSimLog(prev => [{
        time: new Date().toLocaleTimeString(),
        text: 'Select an organization and equipment before running a simulation.',
        type: 'error',
      }, ...prev.slice(0, 19)]);
      return;
    }
    ...
    try {
      ...
      const data = await res.json();
      const targetLabel = selectedSimMachine
        ? `${selectedSimMachine.name} (${selectedSimOrg?.name || selectedSimMachine.organization?.name || 'Unknown'})`
        : 'selected equipment';
      ...
      setSimLog(prev => [{ time: new Date().toLocaleTimeString(), text: `Error: ${e}`, type: 'error' }, ...prev.slice(0, 19)]);
    }
    setSimRunning(null);
  };
  ...
  const filtered = machines
    ...
    .filter(m => filterOrg === 'ALL' || m.organizationId === filterOrg);
  ...
  return (
    <div className="space-y-5 max-w-7xl">
      ...
      {/* Organization Filter dropdown — scalable for many orgs */}
      {orgOptions.length > 1 && (
        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 [background:var(--bg-surface)] border border-[var(--border)] rounded-xl p-3">
          <label htmlFor="hmi-org-filter" className="text-[10px] text-[var(--text-muted)] uppercase tracking-widest font-semibold whitespace-nowrap">
            Organization
          </label>
          <select
            id="hmi-org-filter"
            ...
            className="flex-1 min-w-0 px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--bg-main)] text-[var(--text-primary)] text-xs font-semibold outline-none focus:border-[#635bff]"
          >
            <option value="ALL">All organizations ({machines.length} equipment)</option>
            {orgOptions.map(org => (
              <option key={org.id} value={org.id}>
                {org.name} ({org.count} equipment)
              </option>
            ))}
          </select>
        </div>
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 space-y-3">
          <h2 className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest">
            ...
            {filterOrg !== 'ALL' && <span className="ml-2 normal-case font-normal">· Filtered: {selectedFilterOrgName}</span>}
          </h2>
          {loading ? (
        ...
        <div className="space-y-4">
          ...
          <div className="[background:var(--bg-surface)] border border-[var(--border)] rounded-xl p-4">
            ...
            <div className="space-y-3 mb-4">
              <div>
                <label htmlFor="sim-org-select" className="block text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest mb-1">
                  Target Organization
                </label>
                <select
                  id="sim-org-select"
                  ...
                  onChange={(e) => {
                    ...
                    setSimOrgId(nextOrgId);
                    const firstMachine = machines.find(m => m.organizationId === nextOrgId);
                    setSimMachineId(firstMachine?.id || '');
                  }}
                  disabled={orgOptions.length === 0 || !!simRunning}
                  className="w-full px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--bg-main)] text-[var(--text-primary)] text-xs font-semibold outline-none focus:border-[#635bff] disabled:opacity-50"
                >
                  {orgOptions.length === 0 ? (
              ...
              <div>
                <label htmlFor="sim-machine-select" className="block text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest mb-1">
                  Target Equipment
                </label>
                <select
                  id="sim-machine-select"
                  value={simMachineId}
                  onChange={(e) => setSimMachineId(e.target.value)}
                  disabled={simOrgMachines.length === 0 || !!simRunning}
                  className="w-full px-3 py-2 rounded-lg border border-[var(--border)] bg-[var(--bg-main)] text-[var(--text-primary)] text-xs font-semibold outline-none focus:border-[#635bff] disabled:opacity-50"
                >
                  {simOrgMachines.length === 0 ? (
                    <option value="">No equipment in this organization</option>
                  ) : simOrgMachines.map(machine => (
                ...
                {selectedSimMachine && (
                  <p className="mt-1 text-[10px] text-[var(--text-secondary)]">
                    Actions will target <span className="font-semibold text-[var(--text-primary)]">{selectedSimMachine.name}</span> in {selectedSimOrg?.name || selectedSimMachine.organization?.name || 'Unknown'}.
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-2">
              {[
                ...
                { id: 'reset', label: 'Reset Equipment', color: 'border-gray-500/40 hover:bg-gray-500/10 text-gray-300' },
              ].map(btn => (
                <button key={btn.id} onClick={() => runSim(btn.id)} disabled={!!simRunning || !simOrgId || !simMachineId}
