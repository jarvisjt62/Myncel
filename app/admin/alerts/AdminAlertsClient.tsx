...
export default function AdminAlertsClient({ alerts: initial }: { alerts: Alert[] }) {
  ...
  const orgList = Array.from(new Set(alerts.map(a => a.organization?.name || '—'))).sort();
  ...
  const filtered = alerts.filter(a => {
    ...
    const matchOrg = filterOrg === 'ALL' || (a.organization?.name || '—') === filterOrg;
    return matchSearch && matchSeverity && matchResolved && matchOrg;
  });
  ...
  const resolveAlert = async (alertId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        const updated = data.alert || { isResolved: true, isRead: true, resolvedAt: new Date().toISOString() };
        setAlerts(prev => prev.map(a => a.id === alertId ? { ...a, ...updated } : a));
        if (selected?.id === alertId) setSelected(prev => prev ? { ...prev, ...updated } : prev);
      } else {
        console.error('Failed to resolve alert:', data.error || res.statusText);
      }
    } catch (error) {
