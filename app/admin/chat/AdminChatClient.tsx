...
export default function AdminChatClient({ initialSessions, initialUnreadCount }: AdminChatClientProps) {
  ...
  const notifiedMessageIds = useRef<Set<string>>(new Set(initialSessions.map(s => s.latestMessageId).filter(Boolean) as string[]))

  const activeCount = sessions.length
  const hasUnread = totalUnread > 0
  const unreadSessions = useMemo(() => sessions.filter(s => s.unreadCount > 0), [sessions])

  // Poll for new sessions and unread user messages.
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        ...
        if (res.ok) {
          ...
          const nextSessions: ChatSession[] = data.sessions || []
          setSessions(nextSessions)
          setTotalUnread(data.counts?.totalUnread ?? nextSessions.reduce((sum, s) => sum + s.unreadCount, 0))

          const newlyUnreadSession = nextSessions.find((s) => {
            if (s.id === selectedSession?.id) return false
            if (s.unreadCount <= 0) return false
            if (!s.latestMessageId || s.latestMessageSenderType !== 'USER') return false
            return !notifiedMessageIds.current.has(s.latestMessageId)
          })

          if (newlyUnreadSession?.latestMessageId) {
            notifiedMessageIds.current.add(newlyUnreadSession.latestMessageId)

            if ('Notification' in window) {
              if (Notification.permission === 'granted') {
                new Notification('New live chat message', {
                  body: `${newlyUnreadSession.userName || 'Guest'}: ${newlyUnreadSession.lastMessage || 'sent a message'}`,
                  icon: '/logo.png',
                })
  ...
  useEffect(() => {
    ...
    if (hasUnread) {
      ...
      document.title = originalTitle.includes('New live chat') ? 'Admin Dashboard — Myncel' : originalTitle
    }
  ...
  const loadSession = async (session: ChatSession) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTotalUnread(prev => Math.max(0, prev - session.unreadCount))
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-4">
      <div className={`rounded-xl border px-4 py-3 flex flex-col md:flex-row md:items-center md:justify-between gap-3 ${
        ...
        <div>
          <p className={`text-sm font-bold ${hasUnread ? 'text-red-400' : 'text-[var(--text-primary)]'}`}>
            {hasUnread
              ? `${totalUnread} unread live chat message${totalUnread === 1 ? '' : 's'} need attention`
              : activeCount > 0
                ? `${activeCount} active live chat${activeCount === 1 ? '' : 's'}`
                : 'No active live chats right now'}
          </p>
          <p className="text-xs text-[var(--text-secondary)] mt-1">
            {hasUnread
              ? `Newest unread: ${unreadSessions[0]?.userName || 'Guest'}${unreadSessions[0]?.lastMessage ? ` — “${unreadSessions[0].lastMessage}”` : ''}`
              : 'This page checks for new chat activity every few seconds while it is open.'}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <span className="px-3 py-1.5 rounded-full bg-[var(--bg-hover)] text-[var(--text-primary)] text-xs font-semibold">
            Active: {activeCount}
          </span>
          <span className={`px-3 py-1.5 rounded-full text-xs font-semibold ${
            hasUnread ? 'bg-red-500 text-white' : 'bg-[var(--bg-hover)] text-[var(--text-primary)]'
          }`}>
            Unread: {totalUnread}
          </span>
        </div>
          ...
          {sessions.length === 0 ? (
            ...
            sessions.map((session) => (
              <div
                ...
                className={`p-4 border-b cursor-pointer hover:bg-[#1e2d4a]/50 transition-colors ${
                  selectedSession?.id === session.id ? 'bg-[#1e2d4a] border-l-4 border-l-[#635bff]' : session.unreadCount > 0 ? 'bg-red-500/10 border-l-4 border-l-red-500 border-b-[#1e2d4a]' : 'border-b-[#1e2d4a]'
                }`}
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1 min-w-0">
                    ...
                    {session.lastMessage && (
                      <p className={`text-sm truncate mt-1 ${session.unreadCount > 0 ? 'text-red-300 font-semibold' : 'text-[#635bff]'}`}>
                        {session.latestMessageSenderType === 'USER' ? 'User: ' : session.latestMessageSenderType === 'ADMIN' ? 'Admin: ' : ''}
                        {session.lastMessage}
                      </p>
                    )}
                  </div>
