...
const ROLE_LABELS: Record<string, { label: string; icon: string; desc: string }> = {
  TECHNICIAN: { label: 'Technician', icon: '🔧', desc: 'Diagnose faults, complete repairs, and close maintenance work orders' },
  OPERATOR:   { label: 'Operator',   icon: '🏭', desc: 'Run assigned equipment, monitor HMI status, and report machine issues' },
  EMPLOYEE:   { label: 'Employee',   icon: '👷', desc: 'Submit maintenance requests, view assigned tasks, and follow safety checklists' },
  ADMIN:      { label: 'Admin',      icon: '⚙️', desc: 'Manage team, machines, parts, and work orders' },
  MEMBER:     { label: 'Member',     icon: '👤', desc: 'View dashboards, reports, and shared maintenance updates' },
...
export default function JoinClient({ token }: { token: string }) {
  ...
  const executeCaptcha = async () => {
    ...
    return grecaptcha.execute(siteKey, { action: 'team_join' });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('/api/team/join', {
        ...
        body: JSON.stringify({ token, name: form.name.trim(), password: form.password, captchaToken }),
      });
      const data = await res.json();
