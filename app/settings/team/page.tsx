...
export default function TeamPage() {
  ...
  const getRoleBadgeColor = (role: string) => {
    switch (role) {
      ...
      case 'EMPLOYEE':   return 'bg-cyan-100 text-cyan-700';
      ...
      <div className="rounded-xl border p-6" style={{ background: 'var(--bg-surface)', borderColor: 'var(--border)' }}>
        ...
        <div className="space-y-3 text-sm">
          {[
            ...
            { role: 'ADMIN',      desc: 'Can manage team members, machines, parts inventory, work orders, and reports.' },
            { role: 'TECHNICIAN', desc: 'Best for maintenance staff: diagnose faults, complete repairs, log service notes, and update machine status.' },
            { role: 'OPERATOR',   desc: 'Best for production operators: run assigned equipment, monitor HMI readings, perform start/stop actions, and report issues.' },
            { role: 'EMPLOYEE',   desc: 'Best for general staff: submit maintenance requests, view assigned tasks, follow safety checklists, and track updates.' },
            { role: 'MEMBER',     desc: 'Best for read-only collaborators: view dashboards, machines, work orders, and reports with limited edit permissions.' },
          ].map(item => (
