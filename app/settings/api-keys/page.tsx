...
function ApiKeysPage() {
      ...
      const data = await fetchWithCache(
        ...
        async () => {
          ...
          if (res.ok) {
            ...
            return { keys: json.keys || [], isAdmin: json.isAdmin || false };
          }
  ...
  // Separate own keys and platform keys
  const ownKeys = keys.filter(k => !k.platformManaged);
  const platformKeys = keys.filter(k => k.platformManaged);
  ...
  return (
    <div style={{ backgroundColor: 'var(--bg-page)', color: 'var(--text-primary)' }}>
      <div className="max-w-4xl mx-auto p-6">
        ...
        {/* Toast */}
        {toast && (
          <div className={`fixed bottom-6 right-6 z-[60] px-5 py-3 rounded-xl text-sm font-semibold shadow-xl border ${
            toast.type === 'success'
          ...
          <div className="rounded-2xl overflow-hidden mb-6" style={{ backgroundColor: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
            <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid var(--border)' }}>
              <h2 className="font-semibold" style={{ color: 'var(--text-primary)' }}>
                Shared API Keys
                <span className="ml-2 text-sm font-normal" style={{ color: 'var(--text-secondary)' }}>({platformKeys.length})</span>
              </h2>
            </div>
            <div>
            ...
            <div className="px-6 py-3 text-xs" style={{ color: 'var(--text-muted)', borderTop: '1px solid var(--border)' }}>
              ℹ️ These API keys are configured by the platform admin and are available to all organizations. Contact your admin to make changes.
            </div>
          </div>
        )}
        ...
        <div className="rounded-2xl overflow-hidden mb-6" style={{ backgroundColor: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
          <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid var(--border)' }}>
            <h2 className="font-semibold" style={{ color: 'var(--text-primary)' }}>
              Your API Keys
              <span className="ml-2 text-sm font-normal" style={{ color: 'var(--text-secondary)' }}>({ownKeys.length}/10)</span>
            </h2>
          ...
          {loading ? (
          ...
          ) : ownKeys.length === 0 && platformKeys.length === 0 ? (
            ...
            <div className="px-6 py-8 text-center">
              <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>
                No personal API keys. You can use the platform-managed keys above, or create your own.
              </p>
              <button
                onClick={() => setModal({ kind: 'create' })}
                className="mt-3 bg-[#635bff] hover:bg-[#5248e6] text-white font-semibold px-4 py-2 rounded-lg text-sm transition-colors"
              >
                + New API Key
              </button>
            </div>
...
export default function WrappedPage() {
  return (
    <PlanGate featureKey="feature.api.access" featureName="API Keys" requiredPlan="STARTER">
      <ApiKeysPage />
    </PlanGate>
  );
}