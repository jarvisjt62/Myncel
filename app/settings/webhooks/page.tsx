...
function WebhooksPage() {
  ...
  const fetchWebhooks = useCallback(async (useCache = true) => {
    try {
      const data = await fetchWithCache(
        ...
        async () => {
          ...
          if (res.ok) {
            ...
            return { webhooks: json.webhooks || [], isAdmin: json.isAdmin || false };
          }
          return { webhooks: [], isAdmin: false };
        },
  ...
  useEffect(() => { invalidateCache('webhooks'); fetchWebhooks(false); }, [fetchWebhooks]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        invalidateCache('webhooks'); fetchWebhooks(false);
      } else {
  ...
  const handleToggleActive = async (wh: Webhook) => {
    try {
      ...
      if (res.ok) invalidateCache('webhooks'); fetchWebhooks(false);
    } catch {}
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) { showToast('success', 'Webhook deleted.'); invalidateCache('webhooks'); fetchWebhooks(false); }
      else showToast('error', 'Failed to delete webhook.');
    } catch { showToast('error', 'Failed to delete.'); }
    finally { setDeleting(null); }
  };

  // Separate platform-managed and own webhooks
  const platformWebhooks = webhooks.filter(wh => wh.platformManaged);
  const ownWebhooks = webhooks.filter(wh => !wh.platformManaged);
      ...
      {platformWebhooks.length > 0 && (
        <div>
          <div className="flex items-center gap-2 mb-3">
            <h3 className="text-base font-semibold" style={{ color: 'var(--text-primary)' }}>Shared Webhooks</h3>
          </div>
          <div className="space-y-3 mb-6">
            {platformWebhooks.map(wh => (
              <div key={wh.id} className="rounded-xl border p-5" style={{ background: 'var(--bg-surface)', borderColor: 'var(--border)' }}>
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <span className="text-xs px-3 py-1.5 rounded-lg" style={{ border: '1px solid var(--border)', color: 'var(--text-muted)', background: 'var(--bg-surface)' }}>
                      Managed by Admin
                    </span>
                  </div>
                </div>
            ...
            <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
              ℹ️ These webhooks are configured by the platform admin and receive events for all organizations. Contact your admin to make changes.
            </p>
          </div>
        </div>
      )}

      {/* Own Webhooks Section */}
      <div>
        {ownWebhooks.length > 0 && (
          <h3 className="text-base font-semibold mb-3" style={{ color: 'var(--text-primary)' }}>Your Webhooks</h3>
        )}
        ...
        {loading ? (
        ...
        ) : ownWebhooks.length === 0 && platformWebhooks.length === 0 ? (
          ...
          <div className="rounded-xl border p-6 text-center" style={{ background: 'var(--bg-surface)', borderColor: 'var(--border)' }}>
            <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>
              No personal webhooks configured. You can use the platform-managed webhooks above, or add your own.
            </p>
            <button onClick={openCreate} className="mt-3 px-4 py-2 bg-[#635bff] text-white text-sm font-semibold rounded-lg hover:bg-[#4f46e5] transition-colors">
              + Add Webhook
            </button>
          </div>
        ) : (
...
export default function WrappedPage() {
  return (
    <PlanGate featureKey="feature.api.webhooks" featureName="Webhooks" requiredPlan="GROWTH">
      <WebhooksPage />
    </PlanGate>
  );
}