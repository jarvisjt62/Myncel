...
interface Props {
  orgId: string;
  orgName: string;
  plan: string;           // raw DB plan (e.g. 'TRIAL')
  effectivePlan?: string; // resolved plan for tier checks (active TRIAL → 'GROWTH')
  planData: PlanDef;
  ...
  initialGateways?: Record<string, boolean>;
}
...
// Active TRIAL users resolve to effectivePlan='GROWTH' from the API.
// TRIAL does NOT appear here — the billing API always maps it to GROWTH (active) or TRIAL_RESTRICTED (expired).
...
export default function BillingClient({
  orgId, orgName, plan, effectivePlan: effectivePlanProp, planData, trialEndsAt, trialDaysLeft, isTrialExpired,
  subscriptionStatus, currentPeriodEnd, cancelAtPeriodEnd, hasStripe,
  usage, plans, userRole, stripeConfigured, initialGateways,
}: Props) {
  // Use effectivePlan for tier comparisons: active TRIAL → PROFESSIONAL (full access), expired → TRIAL_RESTRICTED
  const effectivePlan = effectivePlanProp || (plan === 'TRIAL' ? 'PROFESSIONAL' : plan);
  ...
  const [selectedGateway, setSelectedGateway] = useState<string>(() => {
      const gw = initialGateways ?? {};
      if (gw.stripe !== false) return 'stripe';
      if (gw.paypal !== false) return 'paypal';
      if (gw.ach !== false) return 'ach';
      return 'stripe';
    });
  const [checkoutPlanId, setCheckoutPlanId] = useState<string | null>(null);
    const [activeGateways, setActiveGateways] = useState<Record<string, boolean>>(
      initialGateways ?? { stripe: true, paypal: true, ach: true, applePay: true, googlePay: true }
    );
    // ACH payment form step state
    const [achStep, setAchStep] = useState<'select' | 'form' | 'confirm'>('select');
  ...
  useEffect(() => {
    ...
    // Auto-select first enabled gateway if current selection is disabled
    useEffect(() => {
      if (activeGateways[selectedGateway] === false) {
        const firstEnabled = GATEWAYS.find(gw => activeGateways[gw.id] !== false);
  ...
  const currentPlanIdx = PLAN_ORDER.indexOf(effectivePlan);
  ...
  const handleCheckout = async () => {
    ...
    // For ACH, don't close the modal - show the form instead
    if (selectedGateway === 'ach') {
      setAchStep('form');
      return;
    }
  ...
  const handleAchSubmit = async (achData: { accountHolderName: string; routingNumber: string; accountNumber: string; accountType: string }) => {
    if (!checkoutPlanId) return;
    setLoading(checkoutPlanId);
    try {
      const res = await fetch('/api/billing/ach', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (data.demo) {
        showToast('success', `Demo mode: ACH debit authorized for ${checkoutPlanId}. Configure Stripe for ACH processing.`);
        setCheckoutPlanId(null);
        setAchStep('select');
        return;
      }
      showToast('success', 'ACH debit authorization submitted! Your subscription will be activated once the payment clears in 3\u20135 business days.');
      setCheckoutPlanId(null);
      setAchStep('select');
      setTimeout(() => router.refresh(), 2000);
    } catch (err: any) {
      showToast('error', err.message || 'Failed to process ACH payment');
    } finally {
      setLoading(null);
    }
  };
  ...
  const statusBadge = () => {
    if (plan === 'TRIAL_RESTRICTED' || isTrialExpired) return { text: 'Trial Expired', color: '#ef4444', bg: 'rgba(239,68,68,0.1)' };
  ...
  return (
    <div className="space-y-8">
      ...
      {checkoutPlanId && checkoutPlan && (
        <div style={{
        ...
        }} onClick={() => { setCheckoutPlanId(null); setAchStep('select'); }}>
          <div style={{
          ...
          {achStep === 'form' && selectedGateway === 'ach' ? (
            <AchForm
              onSubmit={handleAchSubmit}
              onBack={() => { setAchStep('select'); }}
              loading={!!loading}
            />
          ) : (
          <>
              ...
              {GATEWAYS.filter(gw => activeGateways[gw.id] !== false).map(gw => (
                ...
                >
                  <input
                    ...
                    onChange={() => { setSelectedGateway(gw.id); setAchStep('select'); }}
                    style={{ display: 'none' }}
                  />
                  {/* Radio circle */}
                  <div style={{
      ...
      {(isTrialExpired || plan === 'TRIAL_RESTRICTED') && (
        <div style={{
      ...
      {/* Trial countdown */}
      {plan === 'TRIAL' && !isTrialExpired && trialDaysLeft <= 7 && trialDaysLeft > 0 && (
        <div style={{
          borderRadius: 12, border: '1px solid rgba(245,158,11,0.3)',
      ...
      {/* Current plan card */}
      <div className="rounded-xl border p-6" style={{ background: 'var(--bg-surface)', borderColor: isActiveTrial ? '#f59e0b' : 'var(--border)', boxShadow: isActiveTrial ? '0 0 0 2px rgba(245,158,11,0.15)' : undefined }}>
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            <div className="flex items-center gap-3 mb-1">
              <h3 className="text-lg font-bold" style={{ color: 'var(--text-primary)' }}>
                {isActiveTrial ? 'Free Trial Plan' : `${planData.name} Plan`}
              </h3>
              <span style={{
            ...
            <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>
              {isActiveTrial
                ? 'Full access to all features — experience the complete product free for 30 days'
            ...
            {plan === 'TRIAL' && trialEndsAt && !isTrialExpired && (
              <p className="text-xs mt-2 font-medium" style={{ color: '#f59e0b' }}>
                Trial ends: {new Date(trialEndsAt).toLocaleDateString()} — Choose a plan before it expires
              </p>
            )}
          </div>
      ...
      <div id="plans-section">
        <div className="flex items-center justify-between mb-5 flex-wrap gap-3">
          <div>
            <h3 className="text-lg font-bold" style={{ color: 'var(--text-primary)' }}>
              {isActiveTrial ? 'Choose Your Plan' : 'Available Plans'}
            </h3>
            {isActiveTrial && (
              <p className="text-sm mt-0.5" style={{ color: 'var(--text-secondary)' }}>
                You have full access during your trial. Pick a plan to continue after it ends.
              </p>
            )}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {plans.filter(p => p.id !== 'TRIAL' && p.id !== 'TRIAL_RESTRICTED').map(p => {
            // During active TRIAL: no plan is "current" — user hasn't chosen a paid plan yet
            // For paid plans: show current badge only if they're actually on that paid plan
            const isCurrent = !isActiveTrial && (p.id === effectivePlan || p.id === plan);
            const planIdx = PLAN_ORDER.indexOf(p.id);
            // TRIAL and TRIAL_RESTRICTED are free plans — all paid plans are upgrades, never downgrades
            const isFreePlan = plan === 'TRIAL' || plan === 'TRIAL_RESTRICTED';
            const isDowngrade = !isFreePlan && planIdx < currentPlanIdx;
