  ...
  try {
    ...
    if (!tokenData.ok) {
      console.error('Slack token exchange failed:', {
        ...
        responseMetadata: tokenData.response_metadata,
        needed: tokenData.needed,
        provided: tokenData.provided,
      });
      // Provide specific error redirects for common Slack errors
      if (tokenData.error === 'invalid_client') {
        return NextResponse.redirect(`${baseUrl}/settings/integrations?error=slack_invalid_client`);
      }
      if (tokenData.error === 'invalid_code' || tokenData.error === 'code_already_used') {
        return NextResponse.redirect(`${baseUrl}/settings/integrations?error=slack_invalid_code`);
      }
      if (tokenData.error === 'invalid_redirect_uri') {
        return NextResponse.redirect(`${baseUrl}/settings/integrations?error=slack_redirect_uri`);
      }
