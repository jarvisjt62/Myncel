...
import { checkPlanFeature, hasFeature, getUpgradeMessage } from '@/lib/plan-limits';
...
/**
 * Check if the user's org is the platform admin org.
 * Returns the admin org ID if found, null otherwise.
 */
...
export async function GET(
  ...
  try {
    ...
    // For platform-wide types, only the platform admin org can directly connect.
    // Non-admin orgs should see these as platform-inherited with enable/disable toggle.
    if (PLATFORM_WIDE_TYPES.has(integrationId.toUpperCase())) {
      const [userOrg, adminOrgId] = await Promise.all([
        safeQuery(
          db.user.findUnique({
      ...
      if (userOrg?.organizationId && adminOrgId && userOrg.organizationId !== adminOrgId) {
    ...
    // Check plan feature: QuickBooks and Google Sheets require Growth plan or higher
    const growthOnlyIntegrations = ['quickbooks', 'google_sheets'];
    if (growthOnlyIntegrations.includes(integrationId)) {
      const userForPlan = await safeQuery(
        db.user.findUnique({
          where: { email: session.user.email || '' },
          select: { organizationId: true },
        }),
      ...
      if (userForPlan?.organizationId) {
        const featureCheck = await checkPlanFeature(userForPlan.organizationId, 'feature.integrations.all');
        if (!featureCheck.allowed) {
          return NextResponse.json({
            error: `${integrationId === 'quickbooks' ? 'QuickBooks' : 'Google Sheets'} integration requires the Growth plan or higher. Please upgrade to connect this integration.`,
            code: 'PLAN_FEATURE_REQUIRED',
            feature: 'feature.integrations.all',
    ...
    if (integrationId === 'google_sheets') {
      // Ensure we get a refresh token from Google
      authUrl.searchParams.set('access_type', 'offline');
      authUrl.searchParams.set('prompt', 'consent');
    }

    if (integrationId === 'quickbooks') {
      console.log('[QuickBooks OAuth] Initiating flow with callback:', oauthConfig.callbackUrl);
    }
...
export async function POST(
  ...
  try {
    ...
    // For platform-wide types, only the platform admin org can directly connect.
    if (PLATFORM_WIDE_TYPES.has(integrationId)) {
      const adminOrgId = await getAdminOrgId();
      if (adminOrgId && user.organizationId !== adminOrgId) {
        return NextResponse.json(
          { error: 'This integration is platform-managed. Use the toggle to enable or disable it for your organization.' },
          { status: 403 }
        );
