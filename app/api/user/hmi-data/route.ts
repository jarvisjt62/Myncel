...
export async function GET() {
  try {
    ...
    const sorted = [...machines]
      ...
      .map(m => {
        const latestToken = (m as any).machineDeviceTokens?.[0] ?? null;
        const isGatewayOnline =
          latestToken?.lastSeenAt != null &&
          new Date(latestToken.lastSeenAt) > fiveMinutesAgo;
        return {
          ...m,
          gatewayOnline: isGatewayOnline,
          gatewayLastSeenAt: latestToken?.lastSeenAt?.toISOString() ?? null,
        };
      });
