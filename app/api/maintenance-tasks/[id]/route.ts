...
// PATCH /api/maintenance-tasks/[id] - mark done, update, or full edit
  ...
  try {
    ...
    const updated = await db.maintenanceTask.update({
      ...
      data: {
        ...
        // Full edit fields (admin or user)
        ...
        ...(body.estimatedMinutes !== undefined ? { estimatedMinutes: body.estimatedMinutes !== '' ? Number(body.estimatedMinutes) : null } : {}),
        ...(body.estimatedCost !== undefined ? { estimatedCost: body.estimatedCost !== '' ? Number(body.estimatedCost) : null } : {}),
        ...(body.intervalDays !== undefined ? { intervalDays: body.intervalDays !== '' ? Number(body.intervalDays) : null } : {}),
      },
    ...
    return NextResponse.json({ error: message || 'Failed to update maintenance task' }, { status: 500 });
  }
}
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const message = error instanceof Error ? error.message : String(error);
    return NextResponse.json({ error: message || 'Failed to delete maintenance task' }, { status: 500 });
  }
}
