...
export async function GET(req: NextRequest) {
  try {
    ...
    // Check plan feature: IoT requires Growth plan or higher
    const featureCheck = await checkPlanFeature(orgId, 'feature.iot.sensors');
    if (!featureCheck.allowed) {
      return NextResponse.json({
        error: 'IoT Sensor Integration requires the Growth plan or higher. Please upgrade to access IoT features.',
        code: 'PLAN_FEATURE_REQUIRED',
        feature: 'feature.iot.sensors',
