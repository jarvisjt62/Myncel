...
export async function POST(req: NextRequest) {
  try {
    ...
    const validRoles = ['TECHNICIAN', 'OPERATOR', 'EMPLOYEE', 'ADMIN', 'MEMBER'];
    ...
    // Check plan limit for users (including pending invites)
    const limitCheck = await checkPlanLimit(inviter.organizationId, 'users');
    // Also count pending invites toward the user limit
    const pendingInvitesCount = await db.inviteToken.count({
      where: { organizationId: inviter.organizationId, used: false, expires: { gt: new Date() } },
    });
    const effectiveUserCount = limitCheck.current + pendingInvitesCount;
    if (limitCheck.limit !== null && effectiveUserCount >= limitCheck.limit) {
      return NextResponse.json({
        error: `Team member limit reached. Your ${limitCheck.plan} plan allows up to ${limitCheck.limit} users (including pending invites). Please upgrade your plan to invite more members.`,
        code: 'PLAN_LIMIT_EXCEEDED',
        resource: 'users',
        current: effectiveUserCount,
    ...
    const invite = await db.inviteToken.create({
      data: {
        email: emailLower,
        role: role as 'TECHNICIAN' | 'OPERATOR' | 'EMPLOYEE' | 'ADMIN' | 'MEMBER',
        organizationId: inviter.organizationId,
